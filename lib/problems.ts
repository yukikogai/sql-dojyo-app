...
SELECT name, title, depth FROM subordinates ORDER BY depth, name;`,
  ...
  },
  {
    ...
    description: `月ごとの売上合計（\`total\`）と、当月を含む直近3ヶ月の移動平均（\`moving_avg_3m\`、整数値）を月の昇順（\`month\` は数値）で表示してください。
...
ORDER BY name;`,
  ...
  },
  {
    ...
    description: `各商品の月別売上合計を求め、その商品の全月平均を上回っている月だけを商品名（\`name\`）・月（\`month\`、数値）・売上合計（\`total\`）で表示してください。
商品名・月の昇順で並べること。
...
ORDER BY department;`,
  ...
  },
  {
    ...
    description: `月（\`month\`、数値）・カテゴリごとの売上合計（\`total\`）と、その月内でのカテゴリ売上順位（\`rank\`）を月・順位の昇順で表示してください。
